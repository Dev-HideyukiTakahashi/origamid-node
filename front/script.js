...
async function router() {
  ...
  const hashRoute = location.hash.replace('#', '').split('/').filter(Boolean).shift();
  const route = document.getElementById(hashRoute);

  routes.forEach(item => item.classList.remove('ativo'));

  if (typeof data[hashRoute] === 'function') data[hashRoute](route);
  ...
  if (route) {
    ...
    if (typeof data[hashRoute] === 'function') data[hashRoute]();
  }
}
...
const data = {
      ...
      for (const course of courses) {
        html += /*html*/ `
                <div class='curso-item'>
                  ...
                  <a class='btn' href="#/curso/${escape(course.slug)}">
                    ${escape(course.title)}
                  </a>
                </div>
              `;
        ...
        html += /*html*/ `
              <li>
                <a href="#/aula/${escape(course.slug)}/${escape(lesson.slug)}">
                    ${escape(lesson.title)}
                    <span>
                      <span>${secToMinutes(lesson.seconds)}</span>
  ...
  aula: route => {
    const [_, curso, aula] = location.hash.replace('#', '').split('/').filter(Boolean);
      ...
      let html = /*html*/ `
              <div>
                ...
                <nav id="aula-nav">
                  ${
                    lesson.prev
                      ? `<a class="btn" href="${escape(`#/aula/${curso}/${lesson.prev}`)}">Anterior</a>`
                      : `<span></span>`
                  }
                  ${lesson.completed ? `<span></span>` : renderCompleteButton(lesson.course_id, lesson.id)}
                  ${
                    lesson.next
                      ? `<a class="btn" href="${escape(`#/aula/${curso}/${lesson.next}`)}">Próxima</a>`
                      : `<span></span>`
                  }
                </nav>
              </div>
  ...
  certificados: route => {
    getData('/lms/certificates', certificates => {
      ...
      for (const certificate of certificates) {
        html += /*html*/ `
                  <li>
                    <a class='btn' target="_blank" href="/lms/certificate/${certificate.id}" >${escape(certificate.title)}
                      <span>${escape(certificate.completed).slice(0, 10).split('-').reverse().join('/')}</span>
                  </a>
                  </li>
                `;
      }
...
sendForm('POST', '/auth/login', 'login', async () => {
  ...
  if (user === 'admin') location.hash = '/criar-cursos';
});
...
sendForm('POST', '/lms/courses', 'criar-curso');
...
lessonsForm.addEventListener('submit', async e => {
  ...
  try {
    ...
    const responseLesson = await fetch(BASE + '/lms/lessons', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
